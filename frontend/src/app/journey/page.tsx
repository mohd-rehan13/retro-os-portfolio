...
const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://sunny-illumination-production-7480.up.railway.app/api/v1';
...
export default function JourneyPage() {
  ...
  return (
    <div className="space-y-6 text-brand-cyan font-vt323">
      ...
      {loading ? (
        <div className="text-brand-cyan/40 animate-pulse text-lg tracking-widest py-8">
          [ FETCHING_TIMELINE_DATA... ]
        </div>
      ) : (
        <div className="relative border-l-2 border-brand-cyan/30 ml-4 pl-8 space-y-8">
          {milestones.length === 0 && (
            <div className="text-brand-cyan/30 italic text-sm py-4">
              &gt; NO_JOURNEY_LOGS_FOUND. Initialize system via Admin Panel.
            </div>
          )}
